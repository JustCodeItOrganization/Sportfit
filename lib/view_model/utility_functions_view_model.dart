...
// 1 -> kilo alma
// 2 -> kilo verme
double maintain_weight(
    bool gender, int age, double weight, double height, int fitnesslevel) {
  double rmr = rmr_calculation(
...
double calorie_decision(bool gender, int age, double weight, double height,
    int fitnesslevel, int desicion) {
  double maintainingCalorie =
      maintain_weight(gender, age, weight, height, fitnesslevel);
  if (desicion == 1) {
    return maintainingCalorie * 1.1;
  } else if (desicion == 2) {
    return maintainingCalorie * 0.9;
  }
  return maintainingCalorie;
}
...
double cardio_calorie(
    double mph, bool activity, double body_weight, double minute) {
  // activity == true  -> running
  // activity == false -> bicycling
  Map map_met;
  double met = 0;
  if (activity)
    map_met = running_mets();
  else
    map_met = bicycling_mets();
